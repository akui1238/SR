...
def calc_psnr(sr, hr, dataset=None, scale=1, rgb_range=1):
    ...
    print(f"SR Range: [{sr_min.item()}, {sr_max.item()}]")
    print(f"HR Range: [{hr_min.item()}, {hr_max.item()}]")

     # 确保sr和hr在[0, rgb_range]内
    if not (0 <= sr_min and sr_max <= rgb_range):
         print("Warning: sr values are not within the expected range [0, rgb_range].")
    if not (0 <= hr_min and hr_max <= rgb_range):
         print("Warning: hr values are not within the expected range [0, rgb_range].")
    ...
    # 假设 tensor 是一个 PyTorch 张量，形状为 [1, 3, 604, 700]
    # 这里我们创建一个模拟张量
    tensor = sr.cpu()

    # 将张量的数据类型转换为 uint8
    # 假设原始张量的数据范围是 [-1, 1]，我们需要将其映射到 [0, 255]
    tensor = tensor * 255
    tensor = tensor.clamp(0, 255).to(torch.uint8)

    # 如果张量是批处理形式，取出单个张量
    # 如果张量形状为 [3, 604, 700]，则不需要这一步
    single_tensor = tensor[0]

    # 将 PyTorch 张量转换为 NumPy 数组
    numpy_image = single_tensor.permute(1, 2, 0).numpy()

    # 使用 PIL 库将 NumPy 数组转换为图像
    image = Image.fromarray(numpy_image)

    # 保存图像
    image.save('tensor_to_image_sr.png')
    ...
    # 保存图像
    image.save('tensor_to_image_hr.png')
